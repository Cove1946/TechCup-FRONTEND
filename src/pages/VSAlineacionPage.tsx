...
const MATCH = { local: 'FC KERNEL', visitante: 'LOS DEBUGGERS', fecha: '18 Mar', hora: '3:00 PM', cancha: 'Cancha B' };
    ...
    <div className={styles.field}>
      <div className={styles.fieldInner}>
        ...
        {players.map((p, i) => {
          ...
          return (
            <div key={p.id} className={styles.fieldPlayer} style={{ top: `${slot.top}%`, left: `${slot.left}%` }}>
...
const TeamInfo: React.FC<{ teamName: string; accentColor: string; players: PlayerBase[]; formation: string; stats: typeof LOCAL_STATS }> = ({ teamName, accentColor, players, formation, stats }) => {
  const slots = FORMATION_SLOTS[formation] ?? FORMATION_SLOTS['2-3-1'];
  return (
    <div className={styles.teamInfo}>
      ...
      <div className={styles.statsRow}>
        {[{lbl:'PTS',val:stats.pts},{lbl:'PG',val:stats.pg},{lbl:'PE',val:stats.pe},{lbl:'PP',val:stats.pp},{lbl:'GF',val:stats.gf},{lbl:'GC',val:stats.gc}].map(s => (
          <div key={s.lbl} className={styles.statItem}><span className={styles.statNum}>{s.val}</span><span className={styles.statLbl}>{s.lbl}</span></div>
        ))}
      </div>
    ...
    <MainLayout>
      <div className={styles.page}>
        ...
        <div className={styles.titleRow}>
          ...
          <button className={styles.btnOutline} onClick={() => navigate('/alineacion')}>⚽ Mi Alineación</button>
        </div>
        ...
        <div className={styles.infoRow}>
          <TeamInfo teamName={MATCH.local} accentColor="#15803d" players={LOCAL_PLAYERS} formation={LOCAL_FORMATION} stats={LOCAL_STATS} />
          <div className={styles.infoSep} />
          <TeamInfo teamName={MATCH.visitante} accentColor="#1e3a8a" players={VISITOR_PLAYERS} formation={VISITOR_FORMATION} stats={VISITOR_STATS} />
        </div>
      </div>
    </MainLayout>
  );
